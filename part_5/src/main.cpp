...
void input_accepter_interval_parser_and_queue1_updated(void *param) {
  while (1) {
    if (Serial.available() > 0) {
      ...
      char key_word[] = "delay";
      char *key_word_position = strstr(
          c_recieved, key_word); // search for that word inside the user input

      if (key_word_position != NULL) { // if the keyword 'delay' exists
        int position =
            (int)(key_word_position -
                  c_recieved); // get the index of the keyword 'delay'
        if (position == 0 && *(c_recieved + strlen(key_word)) == ' ') {
          ...
          if (idx != 0) { // means if while loop has been used
            ...
            if (*endptr ==
              ...
              Serial.println(
                  "plase enter a valid interval rate. Ex: delay 100");
            }
          }
...
void setup() {
  ...
  xTaskCreatePinnedToCore(input_accepter_interval_parser_and_queue1_updated,
                          "blinker", 4096, NULL, 2, NULL, app_cpu);
  xTaskCreatePinnedToCore(interval_setter_queue2_updater, "blinker", 4096, NULL,
                          1, NULL, app_cpu);
}
